...
class CreateBookingAction
{
     ...
     * При closeSlot слот помечается закрытым с привязкой к записи (ФТ-8/ФТ-16).
     *
    ...
    public function handle(BookingCode $code, CustomerDraft $draft, BookingSelection $selection): Booking
    {
        return DB::transaction(function () use ($code, $draft, $selection): Booking {
            ...
            // Бронь с сайта занимает час (ФТ-8/ФТ-16): слот закрывается и привязывается к записи —
            // дополнительные записи в слот — только оператором из админки (закрытие не барьер для неё)
            if ($selection->closeSlot) {
