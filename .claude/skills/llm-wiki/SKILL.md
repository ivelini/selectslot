...
Build and maintain a personal knowledge base using LLMs. You manage two directories: `.llm-wiki/raw/` (immutable source material) and `.llm-wiki/wiki/` (compiled knowledge articles). Sources go into raw/, you compile them into wiki articles, and the wiki compounds over time.
...
Three layers, all under `.llm-wiki/` in the user's project root:

**raw/** — Immutable source material. You read, never modify. Organized by topic subdirectories (e.g., `.llm-wiki/raw/machine-learning/`).

**wiki/** — Compiled knowledge articles. You have full ownership. Organized by topic subdirectories, one level only: `.llm-wiki/wiki/<topic>/<article>.md`. Contains two special files:
- `.llm-wiki/wiki/index.md` — Global index. One row per article, grouped by topic, with link + summary + Updated date.
...
Triggers only on the first Ingest. Check whether `.llm-wiki/raw/` and `.llm-wiki/wiki/` exist. Create only what is missing; never overwrite existing files:

- `.llm-wiki/raw/` directory (with `.gitkeep`)
- `.llm-wiki/wiki/` directory (with `.gitkeep`)
- `.llm-wiki/wiki/index.md` — heading `# Knowledge Base Index`, empty body
- `.llm-wiki/wiki/log.md` — heading `# Wiki Log`, empty body
...
2. Pick a topic directory. Check existing `.llm-wiki/raw/` subdirectories first; reuse one if the topic is close enough. Create a new subdirectory only for genuinely distinct topics.

3. Save as `.llm-wiki/raw/<topic>/YYYY-MM-DD-descriptive-slug.md`.
...
- Relative paths from `.llm-wiki/wiki/<topic>/` use `../../raw/<topic>/<file>.md` (two levels up to the wiki root).
...
2. Scan `.llm-wiki/wiki/index.md` entries in other topics for articles covering related concepts.
...
Update `.llm-wiki/wiki/index.md`: add or update entries for every touched article. When adding a new topic section, include a one-line description. The Updated date reflects when the article's knowledge content last changed, not the file system timestamp. See `references/index-template.md` for format.
...
1. Read `.llm-wiki/wiki/index.md` to locate relevant articles.
2. Read those articles and synthesize an answer.
3. Prefer wiki content over your own training knowledge. Cite sources with markdown links: `[Article Title](.llm-wiki/wiki/topic/article.md)` (project-root-relative paths for in-conversation citations; within wiki/ files, use paths relative to the current file).
...
1. Write the answer as a new wiki page. See `references/archive-template.md`. When converting conversation citations to the archive page, rewrite project-root-relative paths (e.g., `.llm-wiki/wiki/topic/article.md`) to file-relative paths (e.g., `../topic/article.md` or `article.md` for same-directory).
...
3. Update `.llm-wiki/wiki/index.md`. Prefix the Summary with `[Archived]`.
4. Append to `.llm-wiki/wiki/log.md`:
   ```
...
**Index consistency** — compare `.llm-wiki/wiki/index.md` against actual `.llm-wiki/wiki/` files (excluding index.md and log.md):
...
**Internal links** — for every markdown link in `.llm-wiki/wiki/` article files (body text and Sources metadata), excluding Raw field links (validated by Raw references below) and excluding index.md/log.md (handled above):
- Target does not exist → search `.llm-wiki/wiki/` for a file with the same name elsewhere.
  - Exactly one match → fix the path.
  - Zero or multiple matches → report to the user.

**Raw references** — every link in a Raw field must point to an existing `.llm-wiki/raw/` file:
- Target does not exist → search `.llm-wiki/raw/` for a file with the same name elsewhere.
  - Exactly one match → fix the path.
  - Zero or multiple matches → report to the user.
...
- `.llm-wiki/wiki/` supports one level of topic subdirectories only. No deeper nesting.
...
- Inside `.llm-wiki/wiki/` files, all markdown links use paths relative to the current file. In conversation output, use project-root-relative paths (e.g., `.llm-wiki/wiki/topic/article.md`).
- Ingest updates both `.llm-wiki/wiki/index.md` and `.llm-wiki/wiki/log.md`. Archive (from Query) updates both. Lint updates `.llm-wiki/wiki/log.md` (and `.llm-wiki/wiki/index.md` only when auto-fixing index entries). Plain queries do not write any files.
- Отвечать в чате и составлять wiki на русском языке
