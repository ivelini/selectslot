...
Схема реализована миграциями и моделями (Laravel 13: атрибуты `#[Fillable]`, `casts()`); enum-классы в `app/Enums/` (+ `WheelRadiusEnum` R13–R21, `CarTypeEnum::bookable()`; enum'ы записей — `app/Enums/Booking/`: `BookingStatusEnum`, `BookingSourceEnum`). Сиды (DatabaseSeeder) наполняют демо-окружение: пользователи `admin@tireslot.local` / `operator@tireslot.local` (пароль `password`), 10 услуг реального прайса (5 работ с полным кубом 135 правил: R13–21 × 3 типа; 5 допработ без правил) + комплекс «Сезонный шиномонтаж» (4 работы), шаблон — все дни 9:00–19:00 (воскресенье рабочее), горизонт сида 60 дней (fallback 30), записи на окно −7…+6 дней от today (расчёт через CalculatePriceAction). Копейки — `unsignedInteger`; производитель работ: `GenerateSlotGridAction` (см. [slot-grid](../domain/slot-grid.md)); расчёт цены — `CalculatePriceAction` (см. [Цены](../domain/pricing.md)).
